...
const manage_data = async (req, res) => {
  ...
  const { approved_data_type, verifying_id, element_id } = req.headers;
  const data_type = req.body?.value_to_update?.data_type;
  // action types = {add_to_set, pull, set}
    ...
    if (action === "add_to_set") {
      try {
        ...
        if (newly_set_element && newly_set_element.modifiedCount > 0) {
          // console.log("Element added successfully:", newly_set_element);
          return res.status(200).json({
            message: `Successfully added to ${req.body.value_to_update?.data_type}`,
            // data: newly_set_element,
          });
        } else {
      ...
      if (newly_removed_element && newly_removed_element.modifiedCount > 0) {
        // console.log("Element removed successfully:", newly_removed_element);
        return res.status(200).json({
...
const search_data_in_all_docss = async (req, res) => {
  // docs have different arrays like the products, sales etc arrays
  // i want to specify the key in request keyn_req which can be any of those arrays.
  // i then want to go over all the elements and pick the ones whose key (search_key) equals the value (search_value)
  const { search_key, search_value, keyn_req } = req.params;
  // keyn_req is short for key_in_requested
  // it is the key that has the data we are requesting
  try {
    const db = await connectToDatabase();
    // Retrieve all documents from the accounts collection
    const documents = await db.collection("accounts").find({}).toArray();
